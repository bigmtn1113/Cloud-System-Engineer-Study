...
### 파일 링크
- 하드 링크: 원본 파일과 동일한 파일을 새로운 파일명으로 생성. ln 명령어를 통해 생성한다.  
- 심볼릭 링크: 원본 파일을 가리키는 새로운 파일을 생성. ln -s 명령어를 통해 생성한다.

### 파일의 구성
파일명 + inode + 데이터 블록

- 파일명: 사용자가 파일에 접근할 때 사용하는 파일의 이름
- inode: 외부적으론 번호(inode 번호)로 표시되고, 내부적으론 파일 상세 정보와 데이터 블록의 주소를 저장

### 하드 링크와 심볼릭 링크 차이
- 심볼릭 링크는 파일의 종류가 l로 표시된다.
- 심볼릭 링크는 파일 이름 뒤에 원본 파일 이름이 표시(파일 이름 -> 원본 파일 이름)된다.
- 하드 링크는 원본 파일과 생성된 파일의 inode 번호가 같지만, 심볼링 링크는 다르다.
- 심볼릭 링크 파일의 내용은 원본 파일의 경로라서 파일 용량이 작다.
- 원본 파일이 삭제되면 심볼릭 링크 파일은 사용할 수 없다.
...
접근 권한이 없으면 '허가 거부'라는 메시지를 출력한다.

ex)
- find /usr -name ls: /usr 디렉터리에서 이름이 ls인 파일을 찾는다.
- find /home -user jeff: /home 디렉터리에서 소유자가 jeff인 모든 파일을 찾는다.
- find ./ -type d: 현재 경로에서 디렉터리를 찾는다.

### 명령어 파일의 위치 찾기
- which: 명령어 파일의 위치를 찾고 그 경로나 alias를 출력. ex) which mv

<br/>

## 파일의 속성
### 파일 소유자의 그룹 이름
ls -l 명령에서 출력되는 그룹명은 파일이 속한 그룹이다.  
사용자가 속한 그룹은 시스템 관리자가 사용자를 등록할 때 결정한다.  
...
\----------  
구분하면 -/---/---/---이 된다.  
파일 타입/ 파일 소유자/ 파일 소유자 소속 그룹/ 기타 사용자

### 접근 권한 변경
- chmod -R: 하위 디렉터리까지 모두 권한 변경

<br/>

## 기본 접근 권한 설정
### 기본 접근 권한
Linux에서는 파일이나 디렉터리를 생성할 때 기본 접근 권한이 자동으로 설정된다.  
일반 파일은 644, 디렉터리는 755의 권한이 주어진다.

### 기본 접근 권한 확인 또는 변경
- umask: 기본 접근 권한 확인
- umask [마스크 값]: 마스크 값을 변경하면 그에 맞춰 기본 접근 권한도 변경

마스크 값 - 022, 077, 027, ...

ex)
- 마스크 022 -> 기본 접근 권한 644(666 - 022), 755(777 - 022)
- 마스크 077 -> 기본 접근 권한 600(666 - 077), 700(777 - 077)

</br>

## 특수 접근 권한
접근 권한은 원래 4자리고, 생략된 맨 앞자리는 특수 권한을 의미한다.  
맨 앞이 1(스티키 비트), 2(SetGID), 4(SetUID)면 특수 접근 권한을 갖고 있단 뜻이다.  
보안상 논란이 있다.

### SetUID
파일이 실행되는 동안만 파일을 실행한 사용자가 아니라 파일 소유자 권한으로 파일을 실행한다.  
접근 권한 맨 앞자리에 4를 설정한다.  
　ex) chmod 4700 파일 이름 -> -rws------.  

SUID가 설정되면 소유자의 실행 권한에 s가 표시(실행 권한이 없으면 S)된다.

### SetGID
파일이 실행되는 동안만 파일을 실행한 사용자가 아니라 파일 소유자 그룹 권한으로 파일을 실행한다.  
접근 권한 맨 앞자리를 2로 설정한다.  
　ex) chmod 2710 파일 이름 -> -rwx--s---.  

SGID가 설정되면 소유 그룹의 실행 권한에 s가 표시(실행 권한이 없으면 S)된다.

### 스티키 비트(sticky bit)
디렉터리에 설정한다.

디렉터리에 스티키 비트가 설정되어 있으면 누구나 이 디렉터리 안에서 파일을 생성할 수 있다.  
파일은 생성자가 소유자로 설정되며, 다른 사용자가 삭제할 수 없다.  
접근 권한 맨 앞자리에 1을 설정한다.  
　ex) chmod 1701 파일 이름 -> -rwx-----t.

스티키 비트가 설정되면 기타 사용자의 실행 권한에 t가 표시(실행 권한이 없으면 T)된다.  
대표적으로 /tmp(drwxrwxrwt.)가 있다.

### 명령어 실행 절차
1\) 명령어 입력  
2\) 현재 디렉터리에서 명령어 존재 여부 확인  
　3-1) 명령어 있으면 실행 권한 확인  
　　4-1) 실행 권한 있으면 사용자 id로 명령 수행  
　　4-2) 실행 권한 없으면 SetUID 확인  
　　　5-1) SetUID 권한 있으면 소유자 id로 명령 수행  
　3-2) 명령어 없으면 환경 변수 PATH 확인  
　　4-1) PATH에 명령어 있으면 실행 권한 확인  
　　　5-1) 실행 권한 있으면 사용자 id로 명령 수행  
　　　5-2) 실행 권한 없으면 SetUID 확인  
　　　　6-1) SetUID 권한 있으면 소유자 id로 명령 수행
